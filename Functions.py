...
def show_main_window(user_id, time_remaining, app):
    ...
    app.protocol("WM_DELETE_WINDOW", do_nothing)  # Prevent closing with 'x' button
    ...
    mouse_sensitivity_window.protocol("WM_DELETE_WINDOW", do_nothing)  # Prevent closing with 'x' button
    mouse_sensitivity_window.overrideredirect(True)  # Remove window decorations and taskbar entry
    ...
    draggable_frame.bind('<B1-Motion>', lambda event: on_window_drag(event, mouse_sensitivity_window))

    mouse_speed_label = tk.Label(draggable_frame, text="Mouse Speed")
    mouse_speed_label.pack()
    mouse_speed_slider = tk.Scale(draggable_frame, from_=1, to=20, orient="horizontal", length=200,
 ...
 # Widget Manager window
    ...
    hardware_usage_window.protocol("WM_DELETE_WINDOW", do_nothing)  # Prevent closing with 'x' button
    hardware_usage_window.overrideredirect(True)  # Remove window decorations and taskbar entry

    time_remaining_var = tk.StringVar()
    time_remaining_var.set(f"Time remaining: {time_remaining} seconds")
    time_remaining_label = tk.Label(hardware_usage_window, textvariable=time_remaining_var)
    time_remaining_label.pack()

    cpu_ram_usage_var = tk.StringVar()
    cpu_ram_usage_label = tk.Label(hardware_usage_window, textvariable=cpu_ram_usage_var)
    cpu_ram_usage_label.pack()

    # Make the Hardware Usage window draggable
    hardware_usage_window.bind('<B1-Motion>', lambda event: on_window_drag(event, hardware_usage_window))
    ...
    hardware_usage_checkbutton = tk.Checkbutton(widget_manager_window, text="Hardware Usage",
                                                variable=hardware_usage_var,
                                                command=toggle_hardware_usage_window)
    hardware_usage_checkbutton.grid(row=0, column=0)

    mouse_sensitivity_checkbutton = tk.Checkbutton(widget_manager_window, text="Mouse Sensitivity",
